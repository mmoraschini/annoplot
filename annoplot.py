        ...
        if event.inaxes:
            if event.name == 'button_press_event':
                ...
                if (self.ax is None) or (self.ax is event.inaxes):
                    self.draw_annotation(event.inaxes, click_x, click_y)

            elif event.name == 'key_press_event' and self.ax is event.inaxes:
                if self.shown_pnt is None:
                    return
...
class PlotAnnotator(Annotator):
    ...
    def __init__(self, xdata, ydata=None, annotations=None, ax=None, annmarkerfacecolor='r', annfacecolor='w',
                 annedgecolor='k'):
        Annotator.__init__(self, ax, annmarkerfacecolor, annfacecolor, annedgecolor)
...
def plot(x, y=None, annotations=None, fig=None, subplotargs=None, fmt=None, **kwargs):
    """
    ...
    fmt: a format string, in the format accepted by plt.plot
    ...
    for i in range(len(x)):
        ...
        if isinstance(x[i], (float, np.floating)):
            format_x = '{:.4f}'.format(x[i])
        else:
            format_x = '{}'.format(x[i])
        if isinstance(y[i], (float, np.floating)):
